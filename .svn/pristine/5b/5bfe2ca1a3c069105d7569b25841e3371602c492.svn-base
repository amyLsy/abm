...
@interface LGAliYunOssUpload()

@property(nonatomic ,strong) LGAliAccessToken *accessToken;
 ...
 */
-(void)uploadfilePath:(NSString *)filePath fileName:(NSString *)fileName bucketName:(NSString *)bucketName completion:(void(^)(BOOL isSuccess ,NSString *url))completion{
...
#warning 待做
    ...
    NSString *endpoint = _accessToken.endpoint;
 ...
 */
-(void)uploadfileData:(NSData *)data fileName:(NSString *)fileName bucketName:(NSString *)bucketName completion:(void(^)(BOOL isSuccess ,NSString *url))completion{
    ...
    OSSClient * client = [[OSSClient alloc] initWithEndpoint:endpoint credentialProvider:credential];
    [self uploadFileData:data fileName:fileName client:client completion:completion];
...
- (void)uploadFile:(NSString *)filePath fileName:(NSString *)fileName client:(OSSClient *)client completion:(void(^)(BOOL isSuccess ,NSString *url))completion{
    
    OSSPutObjectRequest * put = [OSSPutObjectRequest new];
    // 必填字段
    put.bucketName = _accessToken.bucket;
    ...
    [putTask continueWithBlock:^id(OSSTask *task) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!task.error) {
                completion(YES,[NSString stringWithFormat:@"%@.%@/%@",_accessToken.bucket,_accessToken.endpoint,fileName]);
            } else {
...
- (void)uploadFileData:(NSData *)data fileName:(NSString *)fileName client:(OSSClient *)client completion:(void(^)(BOOL isSuccess ,NSString *url))completion{
